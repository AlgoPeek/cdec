...
public:
	...
	{
		int Pos, Len;
		Seg(int _p, int _l): Pos(_p), Len(_l) {}
	};
...
public:
	...
	bool	ReadTextColumns(REF std::wstring16& buffer, REF std::vector<Seg>& segs, WCHAR separator);
	bool	ReadTextColumns(REF std::vector<std::wstring16>& ss, WCHAR separator);
