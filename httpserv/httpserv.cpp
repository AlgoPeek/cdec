...
void Server::Start(UINT port)
{
	...
	d = MHD_start_daemon(
		...
		MHD_OPTION_UNESCAPE_CALLBACK, &OnUnescape, NULL,
		MHD_OPTION_END);
	if (d == NULL)
	...
	else
	{
		...
		else
		{
			if (*upload_data_size != 0)
			{
				...
				if (ctx->m_keepPostData)
				{
					stringx contentLengthStr = ctx->GetHeader(stringx(__X("Content-Length")));
					if (contentLengthStr.Length() != 0) {
						int contentLen = Converter::ToInt32(contentLengthStr);
						if (ctx->m_postRaw.capacity() < contentLen) {
							ctx->m_postRaw.reserve(contentLen);
						}
					}
...
size_t Server::OnUnescape(void *cls, struct MHD_Connection *connection, char *val)
{
	char *rpos = val;
	char *wpos = val;
	char *end;
