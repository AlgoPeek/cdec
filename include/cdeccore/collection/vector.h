...
public:
	...
	typedef ArrayListV<_Ty>					this_type;
...
protected:
	...
	ArrayListV(ref<this_type> rhs)
	{
		m_vecBuffer.insert(m_vecBuffer.end(), rhs->m_vecBuffer.begin(), rhs->m_vecBuffer.end());
	}
	~ArrayListV(){}
...
public:
	...
	{
		for (typename impl_type::iterator it = m_vecBuffer.begin(); it != m_vecBuffer.end(); ++it)
		{
			if (*it == value)
			{
				m_vecBuffer.erase(it);
				return true;
			}
		}
		return false;
