...
inline std::string FromCharToHexString(char ch)
{
	char dig1 = (ch&0xF0)>>4;
	char dig2 = (ch&0x0F);
...
stringx	Encoding::UrlEncode(stringx value)
{
	std::string c = Encoding::get_UTF8()->FromUnicode(value);
	...
	for(int i=0; i<max; i++)
	{
		...
		{
			escaped.append("%");
			escaped.append(FromCharToHexString(c[i]));//converts char 255 to string "ff"
		}
	}

	return Encoding::get_UTF8()->ToUnicode(escaped);
}
