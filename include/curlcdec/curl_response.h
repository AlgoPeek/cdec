...
interface ICurlResponseWriter : public Object
{
	...
	virtual void SetSpeedDownload(double speed) = 0;
	virtual void SetSpeedUpload(double speed) = 0;
...
{
	DECLARE_REF_CLASS(CurlResponse)
...
public:
	...
	void SetSpeedDownload(double speed){ m_speedDownload = speed; }

	void SetSpeedUpload(double speed){ m_speedUpload = speed; }

	void AddHeader(stringx key, stringx value){ m_headers->Insert(key, value); }

	ref<ICurlContentWriter> GetContentWriter(){ return gc_new<StreamCurlContentWriter>(m_stream); }
	...
	inline double GetSpeedDownload() const { return m_speedDownload; }

	inline double GetSpeedUpload() const { return m_speedUpload; }
