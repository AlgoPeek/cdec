...
{
	CreationMode creation = fCreate ? CreationMode::OpenAlways : CreationMode::OpenExisting;
	Open(path, access, share, creation);
}

void FileWrapper::Open(PCWSTR path, AccessMode access, ShareMode share, CreationMode creation)
{
	ASSERT(!IsOpen() && path != NULL && *path != 0);
	m_hFile = ::CreateFileW(path, access, share, NULL, creation, FILE_ATTRIBUTE_NORMAL, NULL);
...
INT64 FileWrapper::Seek(INT64 offset, SeekPosition origin)
{
	ASSERT(IsOpen());
