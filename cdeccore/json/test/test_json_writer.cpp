...
public:
	...
	void TestWriteValues()
	{
		...
		wr->Reset();
		wr->WriteFloat(NULL, .0f);
		stringx strVal = wr->GetString();
		UNITTEST_ASSERT(strVal == __X("0.0"));

		wr->Reset();
		wr->WriteFloat(NULL, 0.0f);
		strVal = wr->GetString();
		UNITTEST_ASSERT(strVal == __X("0.0"));

		wr->Reset();
		wr->WriteFloat(NULL, 0.00f);
		strVal = wr->GetString();
		UNITTEST_ASSERT(strVal == __X("0.0"));

		wr->Reset();
		wr->WriteFloat(NULL, 3.0f);
		strVal = wr->GetString();
		UNITTEST_ASSERT(wr->GetString() == __X("3.0"));

		wr->Reset();
		wr->WriteFloat(NULL, 3.1415926);
		strVal = wr->GetString();
		UNITTEST_ASSERT(wr->GetString() == __X("3.1415926"));
