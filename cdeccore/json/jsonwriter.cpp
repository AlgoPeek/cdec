...
void JsonExpressFormater::WriteExpression(stringx name, ref<JsonNode> node, ref<StringBuilder> sb, int level)
{
	...
	case JSN_Integer:
		...
		break;
	case JSN_Float:
		WriteFloatValue(name, node->FloatValue(), sb, level);
		break;
	case JSN_Boolean:
...
void JsonExpressFormater::WriteFloatValue(stringx name, double value, ref<StringBuilder> sb, int level)
{
	std::stringstream sstrm;
	sstrm << std::setprecision(16) << value;
	stringx strVal = Encoding::get_UTF8()->ToUnicode(sstrm.str());
	if (strVal.IndexOf('.') < 0)
	{
		strVal += __X(".0");
	}

	WriteValue(name, strVal, sb, level);
}
