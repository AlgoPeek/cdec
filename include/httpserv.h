...
class StaticDispatcher : public IUrlDispatcher
{
	DECLARE_REF_CLASS(StaticDispatcher)

protected:
	ref<IRequestHandler>	m_handler;

public:
	StaticDispatcher(ref<IRequestHandler> handler): m_handler(handler) {}
	ref<IRequestHandler> Dispatch(stringx) { return m_handler; }
};
...
protected:	// Callback functions
	...
	static size_t OnUnescape(void *cls, struct MHD_Connection *connection, char *val);
};
